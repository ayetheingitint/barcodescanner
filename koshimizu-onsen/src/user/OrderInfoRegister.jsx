...
const OrderInfoRegisterPage = (props) => {
    ...
    orderinfoobjectList.push(props.orderobjectData);
  console.log('受注登録リストの印刷', props.orderobjectData);
    // バーコードスキャン遷移
    const barcodeScan = () => {
        ...
        localStorage.setItem('orderinfoobjectList', JSON.stringify(props.orderobjectData));
    ...
    const searchByProductName = async () => {
        ...
        if (searchproductname) {
            ...
            try {
                // 商品名検索API呼び出し
                const response = await fetch(`http://127.0.0.1:3000/products/searchproductname?searchproductname=${searchproductname}`)
    ...
    const searchByProductCode = async () => {
        ...
        if (searchproductcode) {
            ...
            try {
                //  商品ｺｰﾄﾞ検索API呼び出し
                const response = await fetch(`http://127.0.0.1:3000/products/searchproductcode?searchproductcode=${searchproductcode}`)
    ...
    const searchByJanCode = async (searchjancode) => {
        if (searchjancode) {
            ...
            try {
                //  Janｺｰﾄﾞ検索API呼び出し
                const response = await fetch(`http://127.0.0.1:3000/products/searchjancode?searchjancode=${searchjancode}`)
    ...
    useEffect(() => {
        ...
        // 受注情報「LocalStorage Data」 あるかどうか確認
        if (JSON.parse(localStorage.getItem('orderInfoProductData'))) {
            // 有場合
            const storedData = JSON.parse(localStorage.getItem('orderInfoProductData'));
            if (storedData.length > 0) {
                setquantity(storedData[0].quantity);
                setsummaryName1(storedData[0].description1);
                setsummaryName2(storedData[0].description2);
                setselectedSummarycodeValue(storedData[0].summary_code);
            }
        }
    ...
    const fetchProductData = async () => {
        try {
            //   商品データ取得API呼び出し
            const response = await fetch('http://127.0.0.1:3000/product/get_product_data');
            if (!response.ok) {
                throw new Error('ネットワークエラーが発生しました。');
            }
            const jsonData = await response.json();
    ...
    const orderRegistration = () => {
        ...
        if (selectedProductOption) {
            ...
            // 入力データ検証チェック
            if (orderinfoobjectList.order_date && orderinfoobjectList.account_type && orderinfoobjectList.warehouse_code && orderinfoobjectList.delivery_date && orderinfoobjectList.delivery_destination_code && orderinfoobjectList.remark_code && orderinfoobjectList.remarks && quantity && summaryName1 && selectedSummarycodeValue) {
                // 入力データ有場合
                const selectedProductItemList = []
                const orderinfoobjectList = []
                selectedProductItemList.push(selectedProductItem);
                orderinfoobjectList.push(props.orderobjectData);
                props.navigator.resetPage({
                ...
                const orderinfoobjectList = []
                // 受注データあるかどうか確認
                if (props.orderobjectData) {
                    orderinfoobjectList.push(props.orderobjectData);
                    props.navigator.resetPage({
                        component: OrderRegisterErrorComponent,
                        props: { selectedProductItem, orderinfoobjectList, orderinfoproductobjectList },
                    });
                }
                else if (props.orderinfoobjectList) {
                    ...
                    orderinfoobjectList.push(JSON.parse(localStorage.getItem('orderinfoobjectList')));
                    props.navigator.resetPage({
                        component: OrderRegisterErrorComponent,
                        props: { selectedProductItem, orderinfoobjectList, orderinfoproductobjectList },
                    });
    ...
    const postProductDataListToApi = async () => {
        ...
        try {
            // 新しい商品を登録API呼び出し
            const response = await fetch("http://127.0.0.1:3000/product/registerproduct", {
                method: 'POST',
                headers: {
