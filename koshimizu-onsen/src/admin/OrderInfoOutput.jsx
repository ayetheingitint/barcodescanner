...
const OrderInfoOutputPage = () => {
    ...
    /* UTF 16 ファイルの列名設定*/
    const customHeader = ['受注日付', '受注№', '得意先ｺｰﾄﾞ', '得意先名１', '納品先ｺｰﾄﾞ', '納品先名', '担当者ｺｰﾄﾞ', '担当者名', '売掛区分', '売掛区分名', '納品日', '倉庫ｺｰﾄﾞ', '倉庫名', '商品ｺｰﾄﾞ', '商品名', '数量', '数量単位ﾞ', '備考ｺｰﾄﾞ', '備考'];
    // 出力ファイル名変更する
    ...
    const writeDataToFile = (data) => {
        try {
            ...
            const tabDelimitedText = data.map(item => `${item.order_date}\t${item.order_number}\t${item.customer_code}\t${item.customer_name}\t${item.delivery_destination_code}\t${item.delivery_destination_name}\t${item.in_charge_code}\t${item.in_charge_name}\t${item.account_type}\t${item.account_type_name}\t${item.delivery_date}\t${item.warehouse_code}\t${item.warehouse_name}\t${item.product_code}\t${item.product_name}\t${item.quantity}\t${item.quantity_unit}\t${item.remark_code}\t${item.remarks}`).join(crlf); // Use CRLF for line endings
